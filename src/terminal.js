...
const NODE_PROFILES = {
  a1: {
    ...
    video: _BASE + 'o.webm',
    content: `● 业务背景与定位
→ 在主导集团科技管理系统期间，面对海量沉淀的非结构化"知识产权与科研交付物"，解决传统 RAG 在跨文档关联和宏观总结时"答非所问、无法全局分析"的痛点，通过AI辅助独立研发的GraphRAG 智能知识库与智能问答
● 核心机制设计：
→ 知识库搭建：为解决开源 GraphRAG 纯自动抽取导致实体命名混乱、业务概念错位的问题，设计AI 泛化推荐到业务人员在线审定（增删改）进行强规则锁定"的人机协同闭环，确保抽取的知识图谱与企业真实业务严密对齐
→ 智能问答引擎：规划"全局社区报告搜索 + 局部图谱三度扩散 + 向量语义匹配"的三路并发路由检索架构，兼顾微观细节精准查找与宏观总结回答
● 项目结果：
→ 通过人机协同审定与三路混合检索机制，彻底解决了企业知识库"不敢用、查不准"的问题；在跨文档复杂提问与宏观总结场景下，回答准确率与用户满意度相比传统 RAG 提升 80% 以上。`,
  },
  a2: {
    ...
    video: _BASE + 'rv.webm',
    content: `● 业务背景与定位
→ 在接手集团"项目管理应用"及日常复杂业务蓝图汇报梳理过程中，针对传统政企遗留系统"代码黑盒、业务文档缺失、变更冲击预警难"的痛点，打造了基于 AI 驱动软件工程（AI4SE）的智能需求审核与资产管理效能平台。

● AI 核心设计与数据纠偏
→ AIGC 逆向工程：主导设计解析流水线，利用大模型逆向解构遗留系统的复杂元数据与底层拓扑，打破新老项目交替时的黑盒困局。
→ HITL（人在回路）动态画布：前端构建类似 ComfyUI 的多层嵌套动态画布，首创"人在回路"数据纠偏机制。允许业务人员以拖拽节点形式可视化修正模型解析的业务拓扑，在零微调成本下，通过极致交互保障入库系统蓝图数据准确率达 100%。

● 智能推演效能提升
→ 将日常业务需求自然语言自动翻译为结构化算子，并在画布动态展示影响范围，大幅提升了业务汇报与开发对齐的效率。`,
  },
  b1: {
    title: '科技管理应用',
