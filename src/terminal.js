...
const NODE_PROFILES = {
  a1: {
    ...
    video: _BASE + 'o.webm',
    content: `● 业务背景与定位
→ 在主导集团科技管理系统期间，面对海量沉淀的非结构化"知识产权与科研交付物"，为解决传统检索"找不到、关联难、幻觉高"的切实痛点，独立全栈研发了企业级 Agent + GraphRAG 通用资产控制台，实现从 B 端痛点到 AI 创新落地的无缝衔接。

● AI 核心架构与落地
→ 混合检索架构：语义层通过递归字符切片将文本向量化并写入 PostgreSQL；关系层开启大模型 JSON Mode，通过 Prompt 工程精准提取科技资产三元组，经 Schema 拦截净化后存入关系表，大幅提升跨项目知识关联准确率并压降幻觉。
→ 增量清洗工作流：基于 n8n 搭建事件驱动的多模态增量清洗流水线，配置定时器异步抓取数据并进行物理隔离比对。

● 产品创新与业务治理
→ 网络拓扑红蓝染色交互，通过动态画布可视化呈现"科研热区"与"战略盲区"；系统级引入算力防刷与熔断机制，有效管控大模型 API 成本。`,
  },
  a2: {
