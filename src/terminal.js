...
→ 实现智能决策闭环，用户点击红色盲区节点可自动触发混合检索并捞取外部趋势。`,
  ...
  a2: {
    ...
    video: _BASE + 'rv.webm',
    content: `● 项目定位
→ 针对传统 B 端遗留系统文档缺失痛点，个人全栈探索的 AI4SE（AI 驱动软件工程）效能工具。

● 产品定义
→ 基于 AIGC 逆向工程流水线的多层嵌套交互画布系统，通过将需求描述翻译为结构化算子，实现代码黑盒解构、变更冲击预警以及需求与代码的双向自动推演。

● 核心突破
→ 低代码元数据逆向解析：主导设计 AIGC 逆向工程流水线，由大模型深度解析遗留系统的复杂元数据与底层架构拓扑，打破新旧系统交替时的代码黑盒。
→ 人在回路（HITL）设计：前端利用开源画布框架构建类似 ComfyUI 的多层嵌套动态交互画布，首创"人在回路（Human-in-the-Loop）"的数据纠偏机制——允许用户以拖动节点的形式可视化审核并修正大模型解析的元数据拓扑，在不依赖高精度微调的前提下，通过极致的交互设计保证入库数据准确率达 100%。
→ 智能需求评审与展示：画布根据缩放展示不同的层级数据。用户在输入框输入新需求描述，系统自动将其翻译为结构化算子，并在画布上对应模块下显示修改内容，评估出需求所影响的现有数据。
→ 画布动态更新：评估后的需求用户确认后可入库，且触发向量同步与全局图谱认知工作流，画布更新为最新的数据并展示。

● 交付成果
→ 成功构建并在线部署了可视化的嵌套交互画布。通过极致的前端性能优化，保障了复杂元数据拓扑节点在画布中缩放与拖拽的低延迟交互体验。`,
  },
  b1: {
    title: '科技管理应用',
